...
class CompilationEngine:
    ...
    def error(self,what):
        print('Error CompilationEngine in line {:d} of {:s}: {:s}'.format(self.tokenizer.line,self.jackfile,what))
        sys.exit(-1)
    ...
    def CompileSubroutineDec(self):
        ...
        self.label=1
        if t in [('keyword','constructor'),('keyword','function'),('keyword','method')]:
            if self.CompileKeyword('constructor') or self.CompileKeyword('function') or self.CompileKeyword('method'):
    ...
    def CompileSubroutineBody(self,subroutine,cfm):
        if self.CompileSymbol('{'):
            while self.CompileVarDec():
                pass
            self.vmfile.writeFunction(self.name+'.'+subroutine,self.table.VarCount('var'))
            if cfm=='method':
    ...
    def CompileStatement(self):
        if self.CompileLetStatement():
            return True
        elif self.CompileReturnStatement():
            return True
        elif self.CompileIfStatement():
            return True
        elif self.CompileWhileStatement():
            return True
        elif self.CompileDoStatement():
            return True

    def CompileLetStatement(self):
        if self.getToken()==('keyword','let'):
            if self.CompileKeyword('let'):
                varName=self.CompileVarName()
                if varName:
                    if self.CompileSymbol('['):
                        if self.CompileExpression():        #rekursiv!
                            if self.CompileSymbol(']'):
                                self.vmfile.writePush(self.segment(self.table.KindOf(varName)),self.table.IndexOf(varName))
                    ...
                    if (ok):
                        if self.CompileSymbol('='):
                            if self.CompileExpression():
                                if self.CompileSymbol(';'):
                                    if array:
                                        self.vmfile.writePop('temp',0)#swap stack
                                        self.vmfile.writePop('pointer',1)
                                        self.vmfile.writePush('temp',0)
                                        self.vmfile.writePop('that',0)
                                    else:
                                        self.vmfile.writePop(self.segment(self.table.KindOf(varName)),self.table.IndexOf(varName))
                                    return True
    ...
    def CompileSubroutineCall(self):
        if self.getToken()[0]=='identifier' and self.nextToken()==('symbol','('):
            ...
            if nargs>0:
                if self.CompileSymbol(')'):
                    self.vmfile.writeCall(self.name+'.'+sr,nargs)#classenname
                    return True
        elif self.getToken()[0]=='identifier' and self.nextToken()==('symbol','.'):
            c=self.CompileClassName()
            nargs=0
            if self.table.TypeOf(c):
                self.vmfile.writePush(self.segment(self.table.KindOf(c)),self.table.IndexOf(c))
    ...
    def CompileKeywordConstant(self):
        t=self.getToken()
        if t[0]=='keyword' and t[1] in ['true','false','null','this']:
            if t[1]=='true':
